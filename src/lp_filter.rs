use embedded_time::{duration::{Microseconds, self}, Clock};
...
impl LPFilter {
    fn new(time_constant: Microseconds<u32>, clock: HalClock) -> Self {
        Self {
            time_constant,
            timestamp_prev: clock.try_now().unwrap().duration_since_epoch().try_into().unwrap(),
            y_prev: 0.0,
        }
    }
    fn run(&mut self, clock: &HalClock) -> f32 {
        let now: Timestamp = clock.try_now().unwrap().duration_since_epoch();
        let dt = now.che
    }
}
